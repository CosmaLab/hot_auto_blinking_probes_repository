function guesses=Guessing_v2(mov,movsz,dfrlmsz,...
    bpthrsh,egdesz,pctile_frame,debugmode,make_guessmovie)
%% BF, 2024-08-12, Modified from Guessing.m, works for a single frame
%% Guessing
...
%
% mov is the image data as a 2D array. 
%
...
if ~pctile_frame
    ...
    bimgmov=bimg((pdsz+1):(movsz(1)+pdsz),(pdsz+1):(movsz(2)+pdsz));
    ...
    bimgmov=logical(bimgmov.*(bimgmov>prctile(bimgmov(bimgmov>0 & ...
        goodfrmmov & repmat(PhaseMasklg,[1,1,1])),bpthrsh)).*repmat(PhaseMasklg,[1,1,1]));
end
...
if ~isempty(centroids)
    guesses=cat(1,guesses,[ones(size(centroids(:,2))),round(centroids(:,2)),round(centroids(:,1))]);
...
if debugmode || make_guessmovie %plot the guesses, for checking parameters
    ...
    if debugmode
        title(['Current frame'],'Interpreter','none')
        keyboard
